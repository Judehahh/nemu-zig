...
fn pmem_read(addr: u32, len: u8) u32 {
    return host_read(guest_to_host(addr), len);
}

fn pmem_write(addr: u32, len: u8, data: u32) void {
    host_write(guest_to_host(addr), len, data);
}

// paddr
fn paddr_read(addr: u32, len: u8) u32 {
    if (in_pmem(addr)) return pmem_read(addr, len);
    out_of_bound(addr);
    unreachable;
}

fn paddr_write(addr: u32, len: u8, data: u32) void {
...
// vaddr
pub fn vaddr_ifetch(addr: u32, len: u8) u32 {
    return paddr_read(addr, len);
}

pub fn vaddr_read(addr: u32, len: u8) u32 {
    return paddr_read(addr, len);
}

pub fn vaddr_write(addr: u32, len: u8, data: u32) void {
    paddr_write(addr, len, data);
}

// host <-> guest
fn guest_to_host(paddr: u32) *const u8 {
...
// host
inline fn host_read(addr: *const u8, len: u8) u32 {
    switch (len) {
...
inline fn host_write(addr: *const u8, len: u8, data: u32) void {
    switch (len) {
